...
const requireAdmin = async (req: any, res: any, next: any) => {
  try {
    // For now, check if user email contains 'admin' - in production, use proper admin roles
    const adminEmails = process.env.ADMIN_EMAILS?.split(',') || ['admin@aiva.com'];
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        GROUP BY u.id, u.firstName, u.lastName, u.email, u.provider, u.providerId, 
                 u.avatar, u.preferences, u.isActive, u.lastLoginAt, u.createdAt, u.updatedAt
